#!/usr/bin/env python3
"""
PPI Network Visualization for 18 PPAR target DEGs.
- Hub genes marked with star shape and golden border
- Non-hub genes as circles colored by regulation direction
- Edge width proportional to STRING confidence score
"""
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
import matplotlib.lines as mlines
import networkx as nx
import numpy as np
import os
from pathlib import Path

# ============================================================
# Data
# ============================================================
HERE = Path(__file__).resolve().parent
outdir = HERE / "figures"

genes_up   = ["CXCL13","CXCR4","ALDH1A3","FABP5","NAMPT","HMOX1","HBEGF","ANGPTL4","LDLR"]
genes_down = ["INSIG1","APOE","BCL2","MOGAT1","PLIN1","ADIPOQ","RBP4","FADS2","PDK4"]

# Hub genes (4-algorithm Rank Sum top 4, natural breakpoint)
hub_genes = {"ADIPOQ", "APOE", "BCL2", "PLIN1"}

# logFC mapping
logfc = {
    "CXCL13": 2.66, "CXCR4": 1.60, "ALDH1A3": 1.58, "FABP5": 1.50,
    "NAMPT": 1.50, "HMOX1": 1.19, "HBEGF": 1.06, "ANGPTL4": 1.02,
    "LDLR": 1.02, "INSIG1": -1.01, "APOE": -1.04, "BCL2": -1.07,
    "MOGAT1": -1.10, "PLIN1": -1.12, "ADIPOQ": -1.34, "RBP4": -1.36,
    "FADS2": -1.65, "PDK4": -1.85
}

# Edges from STRING (score >= 0.4)
edges = [
    ("PDK4","PLIN1",0.421),("PDK4","ADIPOQ",0.440),("PDK4","ANGPTL4",0.739),
    ("HMOX1","ADIPOQ",0.527),("HMOX1","APOE",0.572),("HMOX1","BCL2",0.856),
    ("NAMPT","RBP4",0.695),("NAMPT","ADIPOQ",0.876),
    ("HBEGF","LDLR",0.532),("HBEGF","ADIPOQ",0.767),
    ("APOE","PLIN1",0.403),("APOE","RBP4",0.439),("APOE","ANGPTL4",0.454),
    ("APOE","BCL2",0.491),("APOE","CXCL13",0.557),("APOE","CXCR4",0.558),
    ("APOE","ADIPOQ",0.795),("APOE","LDLR",0.839),
    ("CXCL13","BCL2",0.725),("CXCL13","CXCR4",0.998),
    ("FABP5","PLIN1",0.422),("FABP5","ADIPOQ",0.520),
    ("PLIN1","ANGPTL4",0.405),("PLIN1","ADIPOQ",0.947),
    ("ANGPTL4","ADIPOQ",0.492),
    ("INSIG1","LDLR",0.426),
    ("RBP4","ADIPOQ",0.799),
    ("BCL2","ADIPOQ",0.442),("BCL2","CXCR4",0.759),
    ("CXCR4","ADIPOQ",0.527),
]

# ============================================================
# Build graph
# ============================================================
G = nx.Graph()
all_genes = genes_up + genes_down
for g in all_genes:
    G.add_node(g)

for a, b, s in edges:
    G.add_edge(a, b, weight=s)

# Keep singletons as isolated nodes
singletons = [n for n in G.nodes() if G.degree(n) == 0]
print(f"Network: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges")
print(f"Isolated nodes (no interactions): {singletons}")

# ============================================================
# Layout: spring layout with seed for reproducibility
# ============================================================
# Layout for connected nodes only, then place isolated nodes separately
G_connected = G.copy()
G_connected.remove_nodes_from(singletons)

pos = nx.spring_layout(G_connected, k=2.5, iterations=300, seed=42, weight='weight')

# Place isolated nodes in a row at the bottom
n_iso = len(singletons)
if n_iso > 0:
    # Find the bounding box of connected nodes
    xs = [p[0] for p in pos.values()]
    ys = [p[1] for p in pos.values()]
    x_min, x_max = min(xs), max(xs)
    y_min, y_max = min(ys), max(ys)
    # Place isolated nodes in a horizontal row below the network
    spacing = (x_max - x_min) / max(n_iso - 1, 1) * 0.8
    # Center the isolated nodes horizontally
    total_width = spacing * (n_iso - 1)
    start_x = (x_min + x_max) / 2 - total_width / 2
    y_iso = y_min - 0.22  # below the network, closer
    for i, node in enumerate(sorted(singletons)):
        pos[node] = (start_x + i * spacing, y_iso)

G_clean = G  # use full graph with all nodes

# ============================================================
# Plot
# ============================================================
fig, ax = plt.subplots(1, 1, figsize=(12, 9), facecolor='white')
ax.set_facecolor('white')

# Colors
color_up = "#E74C3C"       # red for up-regulated
color_down = "#3498DB"     # blue for down-regulated
hub_edge_color = "#F1C40F" # golden border for hub genes
hub_marker = "*"
node_marker = "o"

# --- Draw edges ---
edge_weights = [G_clean[u][v]['weight'] for u, v in G_clean.edges()]
# Normalize edge width: 0.4 -> 0.5, 1.0 -> 2.5
min_w, max_w = min(edge_weights), max(edge_weights)
edge_widths = [0.5 + 2.0 * (w - min_w) / (max_w - min_w) for w in edge_weights]

nx.draw_networkx_edges(G_clean, pos, ax=ax,
                       edge_color='#BBBBBB',
                       width=edge_widths,
                       alpha=0.35)

# --- Draw non-hub nodes ---
non_hub_up = [n for n in G_clean.nodes() if n not in hub_genes and n in genes_up]
non_hub_down = [n for n in G_clean.nodes() if n not in hub_genes and n in genes_down]

for n in non_hub_up:
    x, y = pos[n]
    ax.scatter(x, y, s=1000, c=color_up, edgecolors='#333333',
              linewidths=0.8, zorder=3, marker=node_marker)
for n in non_hub_down:
    x, y = pos[n]
    ax.scatter(x, y, s=1000, c=color_down, edgecolors='#333333',
              linewidths=0.8, zorder=3, marker=node_marker)

# --- Draw hub nodes (star shape, golden border, larger) ---
hub_up = [n for n in G_clean.nodes() if n in hub_genes and n in genes_up]
hub_down = [n for n in G_clean.nodes() if n in hub_genes and n in genes_down]

for n in hub_down:  # All 4 hubs are down-regulated
    x, y = pos[n]
    ax.scatter(x, y, s=1200, c=color_down, edgecolors=hub_edge_color,
              linewidths=2.5, zorder=4, marker=hub_marker)

for n in hub_up:
    x, y = pos[n]
    ax.scatter(x, y, s=1200, c=color_up, edgecolors=hub_edge_color,
              linewidths=2.5, zorder=4, marker=hub_marker)

# --- Draw labels ---
# All labels in black, positioned below nodes
for n in G_clean.nodes():
    x, y = pos[n]
    fontsize = 8 if n in hub_genes else 7.5
    fontweight = 'bold' if n in hub_genes else 'normal'
    ax.text(x, y - 0.08, n, fontsize=fontsize, fontweight=fontweight,
            color='black', ha='center', va='top', zorder=5)
ax.set_title("PPI Network of PPAR Target DEGs",
             fontsize=14, fontweight='bold', pad=15)

# --- Legend ---
legend_elements = [
    mlines.Line2D([], [], color=color_up, marker='o', linestyle='None',
                  markersize=10, markeredgecolor='#333333', markeredgewidth=0.8,
                  label='Up-regulated (9 genes)'),
    mlines.Line2D([], [], color=color_down, marker='o', linestyle='None',
                  markersize=10, markeredgecolor='#333333', markeredgewidth=0.8,
                  label='Down-regulated (9 genes)'),
    mlines.Line2D([], [], color=color_down, marker='*', linestyle='None',
                  markersize=12, markeredgecolor=hub_edge_color, markeredgewidth=2.0,
                  label='Hub gene (4 genes)'),
    mlines.Line2D([], [], color='#999999', linewidth=1.5, alpha=0.5,
                  label='Interaction (STRING v12, score > 0.4)'),
]

ax.legend(handles=legend_elements, loc='upper right',
          fontsize=9, frameon=True, fancybox=True, shadow=False,
          edgecolor='#CCCCCC', framealpha=0.9)

# Remove axis
ax.axis('off')

plt.tight_layout()

# Save with explicit padding to prevent clipping
out_png = os.path.join(outdir, "PPI_network_18genes.png")
out_pdf = os.path.join(outdir, "PPI_network_18genes.pdf")
fig.savefig(out_png, dpi=300, bbox_inches='tight', pad_inches=0.3, facecolor='white')
fig.savefig(out_pdf, bbox_inches='tight', pad_inches=0.3, facecolor='white')
plt.close()

print(f"\n✅ Saved:")
print(f"   {out_png}")
print(f"   {out_pdf}")

# Print hub gene summary
print(f"\n📊 Hub genes (4-algorithm Rank Sum, natural breakpoint):")
for g in sorted(hub_genes, key=lambda x: -G_clean.degree(x)):
    d = G_clean.degree(g)
    print(f"   {g}: degree={d}, logFC={logfc[g]}, direction={'DOWN' if g in genes_down else 'UP'}")
